...
class _TodoAddPageState extends State<TodoAddPage> {
  ...
  //todo 생성을 위해 입력 받는 텍스트는 제목, 메모, 시작시간, 종료시간.
  ...
  _saveTodo() {
    ...
    if(DateFormat('hh:mm a').parse(temp.endTime!).compareTo(DateFormat('hh:mm a').parse(temp.startTime!)) == 0 ){
      //만약 임시로 생성된 todo 객체인 temp의 종료시각이 시작시각과 같다면
      ...
      //임시로 생성된 todo 객체인 temp의 종료시각이 시작시각보다 과거라면
      ScaffoldMessenger.of(context)
          .showSnackBar(
          SnackBar(
              backgroundColor: ThemeData.dark().backgroundColor,
      ...
      //임시로 생성된 temp 객체의 요일이 모두 false 값이라면(선택된 요일이 하나도 없다면)
      ScaffoldMessenger.of(context)
          .showSnackBar(
          SnackBar(
              backgroundColor: ThemeData.dark().backgroundColor,
